...
import { clearAllItems, deleteItemsFromCart } from "../../cart/controller/cart.js";
...
export const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  if ((order.status!=="placed"&&order.paymentType=="cash") ||(order.status!=="waitPayment"&&order.paymentType=="card") ) {
    return next(new Error(`can not cancel your order after it been changed to ${order.status} `,{cause:400}))
  }
  const canceledOrder =await Order.findOneAndUpdate({_id:orderId},{status :"canceled",reason,updatedBy :req.user._id})
  ...
  if (order.couponId) {
    await Coupon.updateOne({_id:order.couponId},{$pull:{usedBy:req.user._id}});
  }
...
export const updateByAdmin = asyncHandler(async (req, res, next) => {
  ...
  const updatedOrder =await Order.findOneAndUpdate({_id:orderId},{status,updatedBy:req.user._id})
