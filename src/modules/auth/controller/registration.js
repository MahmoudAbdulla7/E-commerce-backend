...
export const logIn =asyncHandler(async(req,res,next)=>{
    ...
    const accessToken =generateToken({payload:{id:user._id,role:user.role},expiresIn:60*30});
    const refreshToken=generateToken({payload:{id:user._id,role:user.role},expiresIn:60*60*24*30*365});
    user.active="online"
...
export const sendCode =asyncHandler(async(req,res,next)=>{
    ...
    const user =await User.findOneAndUpdate({email},{forgetCode:nanoId()},{ new:true});
    if (!user) {
        return next(new Error("not register account"),{cause:404});
    }
    const html= `
    ...
    return res.status(201).json({message:"Done",forgetCode:user.forgetCode});
});
