...
export const getProducts =asyncHandler(async(req,res,next)=>{
    ...
    for (let i = 0; i < products.length; i++) {
        let calcRating=0;
        for (let j = 0; j < products[i].Review; j++) {
...
export const createProduct =asyncHandler(async(req,res,next)=>{
    const {subcategoryId,brandId,categoryId,name}=req.body;
    ...
    req.body.finalPrice=parseFloat(req.body.price-((req.body.discount||0)/100)*req.body.price).toFixed(2);
    req.body.customId=nanoid();
    const pathOfMainImage=req.files.mainImage[0].path
    const {secure_url,public_id}=await cloudinary.uploader.upload(pathOfMainImage,{folder:`${process.env.APP_NAME}/product/${req.body.customId}`})
...
export const removeProductFromWishList =asyncHandler(async(req,res,next)=>{
    const {productId}=req.params;
    await User.updateMany({_id:req.user._id},{$pull:{wishList:productId}})
